...
import { getClubs, getClubImages, type ClubData } from '@/lib/local-store';
...
export default function ClubDetailPage() {
  ...
  useEffect(() => {
    getClubs().then(async (clubs) => {
      ...
      // 로고/이미지를 별도 키에서 로드
      const id = found.id;
      const imgs = await getClubImages(id);
  ...
  return (
    <div>
      <Link href="/clubs/central" className="inline-flex items-center gap-1.5 text-base text-gray-500 hover:text-gray-800 mb-8 transition-colors">
            ...
            <ul className="space-y-2.5">
              {club.activities.filter(Boolean).map((a, i) => (
                <li key={i} className="flex items-start gap-3 text-base text-gray-600">
                  <span className="w-6 h-6 rounded-full bg-gray-100 text-gray-500 text-sm flex items-center justify-center flex-shrink-0 mt-0.5 font-medium">{i + 1}</span>
                  {a}
                </li>
              ))}
            </ul>
