...
function ProvisionalContent() {
  const searchParams = useSearchParams();
  const initialCategory = searchParams.get('category') || '전체';
  const [selected, setSelected] = useState(initialCategory);
  const [search, setSearch] = useState('');
  const [clubs, setClubs] = useState<ClubData[]>([]);
  useEffect(() => { getProvisionalClubs().then(setClubs); }, []);
    ...
    <div>
      ...
      {clubs.length === 0 ? (
        <div className="text-center py-16 text-gray-400">
          <p className="text-lg mb-1">등록된 가동아리가 없습니다</p>
          <p className="text-sm">관리자 페이지에서 가동아리를 추가해주세요</p>
        </div>
      ) : (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.map((club, idx) => (
              <ScrollReveal key={club.id} animation="zoom-in" delay={Math.min(idx % 6, 5) * 70}>
                <Link href={`/clubs/provisional/${club.id}`}>
                  <div className="bg-white rounded-xl border border-gray-200 p-5 hover:shadow-md hover:border-gray-400 transition-all cursor-pointer h-full">
                    <div className="flex items-start justify-between mb-3">
                      <div className="w-12 h-12 bg-amber-50 rounded-xl flex items-center justify-center flex-shrink-0 overflow-hidden">
                        {club.logoUrl
                          ? <img src={club.logoUrl} alt={club.name} className="w-full h-full object-cover" />
                          : <span className="text-amber-700 font-bold text-sm">{club.name.slice(0, 2)}</span>
                        }
                      </div>
...
export default function ProvisionalPage() {
  return (
    <div>
      <h2 className="text-2xl font-bold text-gray-800 mb-2">가동아리</h2>
      <p className="text-sm text-gray-500 mb-6">중앙동아리 지위 획득을 준비 중인 동아리입니다.</p>
