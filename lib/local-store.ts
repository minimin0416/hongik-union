...
// 이미지 자동 압축 (최대 가로 maxW px, quality 0~1) — WebP 우선 (30-40% 작음)
export function compressImage(file: File, maxW = 1280, quality = 0.80): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      ...
      img.onload = () => {
        ...
        // WebP 미지원 브라우저는 PNG로 fallback됨 — 그 경우 JPEG로 재시도
...
export type ClubData = { id: number; name: string; category: string; room: string; president: string; recruitPeriod: string; meetingSchedule: string; intro: string; desc: string; activities: string[]; targets: string[]; website?: string; websites?: string[]; logoUrl?: string; imageUrl?: string; imageUrls?: string[] };
...
export const saveClubs = (v: ClubData[]) => {
  // 활동 사진(imageUrls)만 별도 키로 분리 — 로고(logoUrl)는 썸네일로 club 데이터에 포함
  const stripped = v.map((c) => {
    const { imageUrls, ...rest } = c;
